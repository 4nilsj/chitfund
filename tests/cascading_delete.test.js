...
describe("Cascading Loan Deletion", () => {
  ...
  it("should delete associated transactions when a loan is deleted", async () => {
    ...
    await db.run(
      "INSERT INTO transactions (member_id, date, type, amount, remarks) VALUES (?, ?, 'contribution', 100000, 'Seed')",
      [memberId, seedDate],
    );
    ...
    const loanRes = await agent.post("/loans/add").send(
    ...
    const disbursement = await db.get(
      "SELECT * FROM transactions WHERE loan_id = ? AND type = 'disbursement'",
      [loanId],
    );
    ...
    await agent.post("/transactions/add").send(
      Object.entries(repaymentData)
        .map(([k, v]) => `${k}=${v}`)
        .join("&"),
    );
    ...
    const deletedLoan = await db.get("SELECT * FROM loans WHERE id = ?", [
      loanId,
    ]);
    expect(deletedLoan).toBeUndefined();

    const remainingTxns = await db.all(
