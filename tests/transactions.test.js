...
describe("Transactions Module", () => {
  ...
  beforeAll(async () => {
    ...
    // Add a member
    const m = await db.run("INSERT INTO members (name, type) VALUES (?, ?)", [
      "Txn Member",
      "member",
    ]);
    memberId = m.lastID;
  ...
  it("should record a generic contribution successfully", async () => {
    ...
    const res = await agent
      ...
      .send(
        `person_id=${memberId}&type=contribution&amount=${amount}&date=${today}&remarks=Initial Contribution`,
      );
  ...
  it("should handle loan repayment updates correctly", async () => {
    ...
    const loanRes = await db.run(
      "INSERT INTO loans (member_id, amount, emi, outstanding, status) VALUES (?, 10000, 1000, 10000, 'active')",
      [memberId],
    );
    ...
    await agent
      .post("/transactions/add")
      .send(
        `person_id=${memberId}&type=repayment&amount=${repayAmount}&date=${today}&remarks=Loan: ${loanId}`,
      );
