...
describe("EMI Schedule Generator Tests", () => {
  ...
  test("should generate EMI schedule for a loan", async () => {
    ...
    db.get = jest.fn().mockResolvedValueOnce(mockLoan);

    db.all = jest.fn().mockResolvedValueOnce(mockRepayments);
  ...
  test("should mark EMI as paid when payment exists", () => {
    ...
    const repayments = [{ date: "2026-03-15", amount: 900 }];
  ...
  test("should generate payment grid for last 6 months", async () => {
    ...
    expect(db.all).toHaveBeenCalledWith(
      expect.stringContaining(
        "SELECT id, name, type, status FROM members WHERE status = 'active'",
      ),
    );
  });
