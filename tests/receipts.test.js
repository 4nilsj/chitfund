...
describe("Receipt Generation", () => {
  ...
  beforeAll(async () => {
    ...
    // Create Member
    await db.run(
      "INSERT INTO members (id, name, contact, type, status) VALUES (1, 'Receipt Tester', '9998887776', 'member', 'active')",
    );

    // Create Transaction
    const res = await db.run(
      "INSERT INTO transactions (member_id, date, type, amount, remarks) VALUES (1, '2025-01-01', 'contribution', 5000, 'Test Receipt')",
    );
  ...
  test("should return 404 for invalid transaction", async () => {
    await request(app).get("/transactions/99999/receipt").expect(404);
  });
});
