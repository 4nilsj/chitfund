...
afterAll(() => {
  Object.assign(fs, jest.requireActual("fs"));
  Object.assign(PDFDocument, jest.requireActual("pdfkit"));
});
...
describe("PDF Passbook Generation Tests", () => {
  test("should create PDF with correct headers", () => {
    ...
    expect(doc.text).toHaveBeenCalledWith("Member Passbook", {
      align: "center",
    });
  });
...
describe("Receipt Integration with Payment Flow", () => {
  test("should update transaction with receipt path after generation", async () => {
    ...
    await mockDb.run("UPDATE transactions SET receipt_path = ? WHERE id = ?", [
      receiptPath,
      contributionResult.lastID,
    ]);
