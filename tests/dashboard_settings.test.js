...
describe("Dashboard Routes", () => {
  ...
  beforeAll(async () => {
    ...
    const m = await db.run(
      "INSERT INTO members (name, type, status) VALUES ('Dash Member', 'member', 'active')",
    );
    memberId = m.lastID;

    // Seed a contribution so dashboard has real data
    await db.run(
      "INSERT INTO transactions (member_id, type, amount, date, remarks) VALUES (?, 'contribution', 5000, '2026-02-04', 'Dash seed')",
      [memberId],
    );
  });
...
describe("Settings Routes - Additional Branches", () => {
  ...
  it("should set monthly contribution override", async () => {
    ...
    const res = await adminAgent
      ...
      .send(`month=${year}-${month}&amount=1500`);
    expect(res.status).toBe(302);
  });
