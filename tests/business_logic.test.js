...
describe("Business Logic Tests", () => {
  ...
  beforeAll(async () => {
    ...
    const res = await db.run(
      "INSERT INTO members (name, type, status) VALUES ('Logic Test User', 'member', 'active')",
    );
    memberId = res.lastID;
  });
  ...
  beforeEach(async () => {
    ...
    await db.run(
      "INSERT INTO transactions (member_id, date, type, amount, remarks) VALUES (?, ?, 'contribution', 100000, 'Seed Capital')",
      [memberId, seedDate],
    );
    ...
    const res = await agent
      .post("/loans/add")
      .send(
        `member_id=${memberId}&amount=${LOAN_AMOUNT}&rate=${LOAN_RATE}&tenure=${LOAN_TENURE}&start_date=${start_date}`,
      );
    ...
    const loan = await db.get("SELECT id FROM loans WHERE member_id = ?", [
      memberId,
    ]);
    loanId = loan.id;
  });
  ...
  describe("Overpayment Prevention", () => {
    it("should reject payment if amount exceeds total outstanding", async () => {
      ...
      const loan = await db.get("SELECT outstanding FROM loans WHERE id = ?", [
        loanId,
      ]);
      ...
      const res = await agent
        .post("/loans/repay")
        .send(
          `loan_id=${loanId}&amount=${payAmount}&date=${todayStr}&month_for=Full Closure`,
        );

      // Should redirect with error
      expect(res.status).toBe(302);
      expect(res.header.location).toContain(
        "error=Payment%20exceeds%20outstanding",
      );
  ...
  describe("Partial Payment Tracking", () => {
    it("should accept partial payment and keep loan active", async () => {
      ...
      const res = await agent
        .post("/loans/repay")
        .send(
          `loan_id=${loanId}&amount=${partialAmount}&date=${todayStr}&month_for=June 2026`,
        );

      // Should succeed
      expect(res.status).toBe(302);
      expect(res.header.location).toContain(
        "msg=Repayment%20recorded%20successfully",
      );
  ...
  describe("Early Closure", () => {
    it("should close loan immediately upon full repayment", async () => {
      ...
      const res = await agent
        .post("/loans/repay")
        .send(
          `loan_id=${loanId}&amount=${fullAmount}&date=${todayStr}&month_for=Full Settlement`,
        );
  ...
  describe("Interest Accuracy", () => {
    it("should calculate total payable accurately (Flat Rate)", async () => {
      ...
      const loan = await db.get(
        "SELECT outstanding, emi FROM loans WHERE id = ?",
        [loanId],
      );
