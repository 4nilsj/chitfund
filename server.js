...
if (process.env.NODE_ENV === "production" && !process.env.SESSION_SECRET) {
  console.error(
    "FATAL: SESSION_SECRET environment variable is not set. Set it in your .env or server environment before starting.",
  );
  process.exit(1);
}
...
const sessionSecret =
  process.env.SESSION_SECRET || require("crypto").randomUUID();
app.use(
  session({
...
const csrfGlobalMiddleware =
  process.env.NODE_ENV === "test"
    ? (req, res, next) => {
        // No-op in test, but still attach a dummy csrfToken function
        req.csrfToken = () => "";
        next();
      }
    : (req, res, next) => {
        // Skip CSRF for multipart endpoints where Multer must run first
        if (req.path === "/loans/repay" && req.method === "POST") {
          req.csrfToken = () => "";
          return next();
        }
        // Generate new token if not in session, or validate existing token
...
app.use(async (req, res, next) => {
  try {
    const setting = await db.get(
      "SELECT value FROM settings WHERE key = 'fund_name'",
    );
