...
router.get("/", async (req, res) => {
  try {
    ...
    const loansWithPaymentInfo = await Promise.all(
      loans.map(async (loan) => {
        ...
        const interestRatio =
          totalPayable > 0 ? totalInterest / totalPayable : 0;
        ...
        if (repayments.length > 0) {
          ...
          lastPaidMonth = latestRepayment.remarks
            ? latestRepayment.remarks.replace("EMI for ", "").split(" - ")[0]
            : null;
    ...
    const totalInterestOnPage = loansWithPaymentInfo.reduce(
...
router.post("/add", canWrite, validateLoan, async (req, res) => {
  ...
  if (!errors.isEmpty()) {
    return res.redirect(
      buildRedirectUrl("/loans", { error: errors.array()[0].msg }),
    );
  }
  ...
  try {
    ...
    if (P > currentBalance) {
      return res.redirect(
        buildRedirectUrl("/loans", {
          error: `Insufficient fund balance. Available: ${formatCurrency(currentBalance)}`,
        }),
      );
    }
    ...
    try {
      ...
      await db.run(
        ...
        [
          ...
          P,
          `Loan Disbursement: L${loanRes.lastID}`,
          loanRes.lastID,
        ],
      );
  ...
  async (req, res) => {
    try {
      ...
      if (payAmount > loan.outstanding) {
        return res.redirect(
          buildRedirectUrl("/loans", {
            error: `Payment exceeds outstanding amount: ${formatCurrency(loan.outstanding)}`,
          }),
        );
      }
      ...
      if (existingRepayment) {
        return res.redirect(
          buildRedirectUrl("/loans", {
            error: `EMI for ${month_for} already paid`,
          }),
        );
      }
      ...
      try {
        ...
        await db.run(
          ...
          [newOutstanding, newStatus, loan_id],
        );
...
router.get("/:id/schedule/pdf", async (req, res) => {
  try {
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="Loan_${id}_Schedule.pdf"`,
    );
    doc.pipe(res);
    ...
    doc
      .fontSize(20)
      .text(res.locals.fundName || "Chit Fund", { align: "center" });
    doc
      .fontSize(16)
      .text(`Repayment Schedule - Loan #${loan.id}`, { align: "center" });
    doc.moveDown();
...
router.post("/waive", isAdmin, async (req, res) => {
  try {
    ...
    console.error(err);
    res.redirect(
      buildRedirectUrl("/loans", { error: "Error processing waiver" }),
    );
  }
});
