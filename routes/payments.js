...
router.post("/monthly", async (req, res) => {
  ...
  try {
    ...
    if (obligation.alreadyPaid) {
      return res.redirect(
        buildRedirectUrl("/", { error: "already_paid_month" }),
      );
    }
      ...
      if (contributionToPay > 0) {
        const contributionResult = await db.run(
          ...
          [
            ...
            paymentDate,
            contributionToPay,
            `Monthly contribution for ${month}${contributionToPay < obligation.contributionAmount ? " (Partial)" : ""}`,
            month,
          ],
        );
          ...
          if (loanPayment > 0) {
            await db.run(
              ...
              [
                ...
                paymentDate,
                loanPayment,
                `EMI for Loan #${loan.id} - ${month}${loanPayment < loan.emi ? " (Partial)" : ""}`,
                batchId,
                loan.id,
              ],
            );
