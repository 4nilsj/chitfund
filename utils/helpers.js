...
async function calculateMonthlyObligation(memberId, month) {
  try {
    ...
    const member = await db.get("SELECT type FROM members WHERE id = ?", [
      memberId,
    ]);
    ...
    if (!isPublic) {
      ...
      const monthOverride = await db.get(
        "SELECT value FROM settings WHERE key = ?",
        [`contribution_${month}`],
      );

      // 2. Check for global default
      const globalDefault = await db.get(
    ...
    const activeLoans = await db.all(
      "SELECT id, emi, amount, outstanding, interest_waived FROM loans WHERE member_id = ? AND status = 'active'",
      [memberId],
    );
...
module.exports = {
  ...
  getFundBalance: async () => {
    try {
      ...
      const openingBalance = openingSetting
        ? parseInt(openingSetting.value)
        : 0;
